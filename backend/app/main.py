...
def analyze(payload: AnalyzeIn):
    workflow = Workflow(); result = workflow.run(parse_profile((payload.profile or ProfileIn(**PROFILE)).model_dump()), payload.job_id, payload.free_only); ANALYSES[result["id"]] = result; return result

@app.get("/api/analysis/{analysis_id}")
def analysis(analysis_id: str): return ANALYSES.get(analysis_id, {"error": "Analysis not found"})
...
def roadmap(analysis_id: str, free_only: bool = False):
    ...
    if not result: return {"error": "Analysis not found"}
...
def agent_logs(analysis_id: str): return {"analysis_id": analysis_id, "logs": ANALYSES.get(analysis_id, {}).get("logs", [])}
